...
class DisplaySavedImage extends Component {
    ...
    Share = async () => {
        ...
        const shareOptions = Platform.OS === "ios" ? {
            title: "Hık Demiş",
            url: "assets-library://asset/asset.JPG?id=" + result[1] + "&ext=JPG",
        ...
        return (
            <SafeAreaView style={styles.container}>
                <ScrollView showsVerticalScrollIndicator={false}>
                    <Image source={{uri: image.uri}} style={Platform.OS === 'ios' ? [styles.imageStyle, {
                        width: DEVICE_WIDTH * 0.85,
                        height: DEVICE_WIDTH * 0.85 * (image.height / image.width)
                    }] : [styles.imageStyle, {
                        width: DEVICE_WIDTH * 0.85,
